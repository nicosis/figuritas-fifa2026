...
export const useAlbumStore = () => {
      ...
      if (user.value) {
        ...
        if (guestDataStr) {
          try {
            ...
            guestStickersCount = Object.values(guestData).reduce((sum, val) => sum + val, 0)
          } catch (e) {
        ...
        if ((!remoteData || remoteData.length === 0) && guestStickersCount > 0) {
          console.log('Migrating guest progress to Supabase for the new user...')
  ...
  const updateSticker = async (countryId, number, quantity) => {
    ...
    if (user.value) {
      try {
        ...
        syncError.value = 'Guardado localmente. Error de conexión con Supabase.'
      }
    }
  }
  ...
  const stats = computed(() => {
    ...
    const progressPercent = totalStickersCount > 0 
